...
4      | 1    | Version            | 0x01
5      | 1    | Sub-version        | 0x01
6      | 1    | Body size (DWORDs) | (total_length / 4)
7      | 1    | Packet Type        | 0x00-0x0e
8      | 2    | Nonce              | Big-endian
...
### Packet Format
```
Total: 282 bytes (14 Ethernet + 12 Header + 256 Payload)

[14 bytes] Ethernet header
[12 bytes] Xbox header (Type 0x02, body size = 0x43 DWORDs)
[256 bytes] Payload (see structure below)
```
...
-------------|------|-------------------------|------------------
0-19         | 20   | HMAC-SHA1 signature     | Computed
20-103       | 84   | Copyright string        | Static firmware
104-135      | 32   | Adapter name            | "Xbox Wireless Adapter (MN-740)"
136-167      | 32   | Firmware version        | "1.0.2.26 Boot: 1.3.0.06"
168          | 1    | Capability flags        | 0x06
169          | 1    | Unknown flag            | 0x07
170-173      | 4    | Unknown data            | 0x00 0x00 0x0f 0xfe
174-179      | 6    | BSSID (AP MAC)          | FUN_80076d50()
180          | 1    | Signal strength (0-255) | FUN_8009914c()
181          | 1    | Link quality (SNR)      | FUN_8009913c()
182-185      | 4    | IP address (big-endian) | FUN_80098ff8()
186-217      | 32   | WPA/WPA2 PMK Cache      | Fast Roaming buffer
218          | 1    | Wireless mode           | FUN_80098a64()
219          | 1    | SSID length             | 0-32
220-251      | 32   | SSID string             | FUN_800985d0()
252-255      | 4    | Connection status       | Connection state
256-260      | 5    | DHCP/DNS metadata       | See below
261          | 1    | WiFi channel            | FUN_800985a0()
262-281      | 20   | Reserved                | Padding
```
...
### Brief Description
Returns discovered networks in 64-byte slots.

### Packet Format
```
Total: Variable (14 Ethernet + 12 Header + 1 Count + N×64 Slots)

[14 bytes] Ethernet header
[12 bytes] Xbox header (Type 0x04, body size = variable)
[1 byte]   Network count (N)
[N×64 bytes] Network slots
```
...
1      | N×64  | Network slot array
```

### Network Slot Structure (64 bytes each)
```
Offset | Size | Field              | Description
-------|------|--------------------|------------------
0-5    | 6    | BSSID              | AP MAC address
...
45-52  | 8    | Supported rates    | 802.11 rates
53-60  | 8    | Padding            | Zeros
61-63  | 3    | Next BSSID prefix  | First 3 bytes of next network
```
...
**Purpose**: Allows Xbox to spoof the MAC address of a previously registered PC.
...
6. Adapter → Xbox:  NETWORKS_LIST_RESPONSE (Type 0x04)
   - Array of 64-byte network slots
...
- Check each network slot is exactly 64 bytes
