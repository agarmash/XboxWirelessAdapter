...
### Brief Description
Returns discovered networks in 59-byte slots.
...
-------|-------|-------
0      | 1     | Network count
1      | N×59  | Network slot array
```

### Network Slot Structure (59 bytes each)
```
Offset | Size | Field              | Description
-------|------|--------------------|------------------
...
```
1500 - 12 - 1 = 1487 bytes/ 59 = 25 Slots
```
...
6. Adapter → Xbox:  NETWORKS_LIST_RESPONSE (Type 0x04)
   - Array of 59-byte network slots
...
- Check each network slot is exactly 59 bytes
...
0.8s                  <──────── Type 0x04              Results ready
                                (15 networks × 59 bytes) //safe limit
                                (15 networks × 59 bytes + one truncated 16th network @ 19 bytes) //actual format due to firmware bug
